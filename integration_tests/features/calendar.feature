...
Feature: microservices for calendar implementation
  ...
  Scenario: Api create new event
    When I send "POST" request to "http://api:7766/event" with "application/json" data:
    """
    {
        "title": "test",
  ...
  Scenario: Api update event
    When I send "PUT" request to "http://api:7766/event/:id" with "application/json" data:
    """
    {
  ...
  Scenario: Api get all event list
    When  I send "GET" request to "http://api:7766/events"
    Then The response code should be 200
  ...
  Scenario: Scheduler sent event
    When I change time_send_notify for event
